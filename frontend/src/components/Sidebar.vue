<template>
  <div class="w-1/2 md:w-1/3 lg:w-64 fixed md:top-0 md:left-0 h-screen lg:block bg-gray-100 border-r z-30" :class="sideBarOpen ? '' : 'hidden'" id="main-nav">
    ...
    <div class="mb-4 px-4">
      ...
      <div class="w-full flex items-center text-blue-400 h-10 pl-4 bg-gray-200 hover:bg-gray-200 rounded-lg cursor-pointer" @click="navigateTo('dashboard')">
...
export default {
  ...
  methods: {
    ...
    navigateToNode(node_id) {
      this.$router.push({ name: 'NodeDetail', params: { nodeId: node_id } });
    }
  },
  created() {
    this.fetchNodes();
