  ...
  <div id="home">
    <div v-if="!isArduinoAdded" class="add-arduino fixed inset-0 flex items-center justify-center z-50">
      <div class="modal-content text-center bg-white p-10 rounded shadow-lg w-1/2 h-1/2">
        <h2 class="text-2xl mb-4" v-if="!showModal">센서 추가</h2>
    ...
    <div :style="{ filter: isArduinoAdded ? 'none' : 'blur(10px)' }">
      ...
      <div class="flex flex-wrap -mx-3 mb-20">
        <div
          ...
          :key="arduino.id"
          @click="selectArduino(arduino.node_id)"
        >
          <div
            class="w-full bg-white border text-blue-400 rounded-lg flex items-center p-6 mb-6 xl:mb-0 cursor-pointer"
            :class="{ 'bg-gray-200': selectedArduino === arduino.node_id }"
          >
...
export default {
  ...
  methods: {
    ...
    selectArduino(node_id) {
      this.selectedArduino = node_id;
